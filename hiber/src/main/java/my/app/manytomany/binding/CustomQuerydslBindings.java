...
/**
 * {@link CustomQuerydslBindings} allows definition of path specific bindings.
 *
 * <pre>
 * <code>
 ...
 * <p>
 * {@link CustomQuerydslBindings} are usually manipulated using a {@link CustomQuerydslBinderCustomizer}, either implemented
 * directly or using a default method on a Spring Data repository.
 *
 * @author Christoph Strobl
 * @author Oliver Gierke
 * @see CustomQuerydslBinderCustomizer
 * @since 1.11
 */
public class CustomQuerydslBindings {

    private final Map<PathOperatorPair, PathAndBinding<?, ?>> pathSpecs;
    ...
    /**
     * Creates a new {@link CustomQuerydslBindings} instance.
     */
    public CustomQuerydslBindings() {
     ...
     * @see #including(Path...)
     */
    public final CustomQuerydslBindings excludeUnlistedProperties(boolean excludeUnlistedProperties) {
    ...
    public <S extends Path<? extends T>, T> Optional<MultiValueBinding<S, T>> getBindingForPathAndOperator(Ops operator, PathInformation path) {

        Assert.notNull(path, "PropertyPath must not be null!");

        PathAndBinding<S, T> pathAndBinding = (PathAndBinding<S, T>) pathSpecs.get(PathOperatorPair.from(operator, path.toDotPath()));
    ...
    private static String toDotPath(Optional<Path<?>> path) {
        return path.map(CustomQuerydslBindings::fromRootPath).orElse("");
    }
    ...
    public class PathBinder<P extends Path<? extends T>, T> {
         ...
         * @return
         */
        public void firstOptional(Ops operator, OptionalValueBinding<P, T> binding) {

            Assert.notNull(binding, "Binding must not be null!");

            all(operator, (path, value) -> binding.bind(path, Optionals.next(value.iterator())));
        }

        public void first(Ops operator, SingleValueBinding<P, T> binding) {

            Assert.notNull(binding, "Binding must not be null!");
            all(operator, (path, value) -> Optionals.next(value.iterator()).map(t -> binding.bind(path, t)));
        }
         ...
         * @return
         */
        public void all(Ops operator, MultiValueBinding<P, T> binding) {

            Assert.notNull(binding, "Binding must not be null!");

            paths.forEach(path -> registerBinding(operator, PathAndBinding.withPath(path).with(binding)));
        }

        protected void registerBinding(Ops operator, PathAndBinding<P, T> binding) {
            CustomQuerydslBindings.this.pathSpecs.put(PathOperatorPair.from(operator, toDotPath(binding.getPath())), binding);
        }
    }
    ...
    public class AliasingPathBinder<P extends Path<? extends T>, T> extends PathBinder<P, T> {
         ...
         */
        public void withDefaultBinding(Ops operator) {
            registerBinding(operator, PathAndBinding.withPath(path));
        }
        ...
        protected void registerBinding(Ops operator, PathAndBinding<P, T> binding) {
            ...
            super.registerBinding(operator,binding);

            if (alias != null) {
                CustomQuerydslBindings.this.pathSpecs.put(PathOperatorPair.from(operator, alias), binding);
                CustomQuerydslBindings.this.aliases.add(alias);
                CustomQuerydslBindings.this.blackList.add(toDotPath(binding.getPath()));
            }
        }
    }
    ...
    public final class TypeBinder<T> {
        ...
        public <P extends Path<T>> void all(MultiValueBinding<P, T> binding) {
            ...
            CustomQuerydslBindings.this.typeSpecs.put(type, PathAndBinding.<T, P>withoutPath().with(binding));
        }
    }
    ...
    private static class PathAndBinding<P extends Path<? extends T>, T> {
        ...
        Optional<Path<?>> path;
        @NonNull
        Optional<MultiValueBinding<P, T>> binding;
